...
public class BenchmarkActorRingIntra extends BenchmarkSampleActor4j {
	...
	public BenchmarkActorRingIntra(BenchmarkConfig config) {
		...
		benchmark.start((timeMeasurement, iteration) -> {
			...
			for (UUID sender : ringList)
				system.send(ActorMessage.create(null, Actor.POISONPILL, null, sender)); // stop all actors from parent
	...
	public static void main(String[] args) {
		new BenchmarkActorRingIntra(new BenchmarkConfig(100, 10, 60, 8, 1, String.valueOf(1_000_000))); // 10 + 60 iterations!
	}
}
